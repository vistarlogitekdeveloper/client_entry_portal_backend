const service = require('./ho_certification.service');
...
exports.getOne = async (req, res) => {
  try {
    ...
    if (!data) return res.status(404).json({ success: false, message: 'Certification not found' });
    res.json({ success: true, data });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
exports.uploadFiles = async (req, res) => {
  try {
    ...
    for (const file of req.files) {
      const fileRecord = await service.addFile(certificationId, file.originalname, file.mimetype, file.buffer);
